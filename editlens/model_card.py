...
| [`{FAMILY['B']}`](https://huggingface.co/{{{{NS}}}}/{FAMILY['B']}) | **Tiny OOD adapter** (a few MB) that snaps onto a frozen EditLens-Qwen3 (search HF: `editlens qwen3 repro`) checkpoint to add an anomaly / human-likeness score — no backbone training. | You already run EditLens and want to add an OOD score cheaply. |
